...
class IDGenerator(object):
    ...
    def create_id(self, prefix="guid"):
        ...
        ns_prefix = next(iter(list(self.namespace.values())))
...
def _set_cybox_namespace(namespace):
    uri = next(iter(list(namespace.keys())))
    prefix = next(iter(list(namespace.values())))
...
def get_id_namespace():
    ...
    return next(iter(list(_get_generator().namespace.keys())))


def get_id_namespace_alias():
    """Returns the namespace alias assoicated with generated ids"""
    return next(iter(list(_get_generator().namespace.values())))
