...
class NamespaceInfo(object):
    ...
    def _fix_example_namespace(self):
        ...
        """
        ex_api_ns = list(idgen.EXAMPLE_NAMESPACE.keys())[0]
    ...
    def _check_namespaces(self, ns_dict):
        ...
        """
        for prefix, namespaces in list(ns_dict.items()):
            if len(namespaces) == 1:
                continue
    ...
    def _finalize_namespaces(self, ns_dict=None):
        ...
        user_namespaces = {}
        for ns, alias in list(ns_dict.items()):
        ...
        collected_prefixed = dict(iter(list(self._collected_namespaces.items())))
        ...
        namespace_dicts = itertools.chain(
            iter(list(self._BASELINE_NAMESPACES.items())),
            iter(list(self._input_namespaces.items())),
            iter(list(collected_prefixed.items())),
            iter(list(collected_unprefixed.items())),
            iter(list(user_namespaces.items()))
        )
        ...
        flattened = {}
        for alias, ns_set in list(ns_dict.items()):
    ...
    def _finalize_schemalocs(self, schemaloc_dict=None):
        ...
        # modify the original.
        if schemaloc_dict:
            schemaloc_dict = dict(iter(list(schemaloc_dict.items())))
        else:
            schemaloc_dict = {}
        ...
        for ns, loc in list(self._input_schemalocs.items()):
        ...
        nsset = set(self.finalized_namespaces.values())
        for ns in nsset:
    ...
    def _finalize_binding_namespaces(self):
        ...
        binding_namespaces = {}
        for alias, ns in list(self.finalized_namespaces.items()):
...
class NamespaceParser(object):
    ...
    def get_schemaloc_str(self, schemaloc_dict):
        ...
        pairs = sorted(schemaloc_dict.items())
...
DEFAULT_STIX_NAMESPACES  = dict(
    itertools.chain(
        iter(list(DEFAULT_CYBOX_NAMESPACES.items())),
        iter(list(XML_NAMESPACES.items())),
        iter(list(DEFAULT_STIX_NS_TO_PREFIX.items())),
        iter(list(DEFAULT_EXT_TO_PREFIX.items()))
    )
)

#: Prefix-to-namespace mapping of the `DEFAULT_STIX_NAMESPACES` mapping
DEFAULT_STIX_PREFIX_TO_NAMESPACE = dict(
    (alias, ns) for ns, alias in list(DEFAULT_STIX_NAMESPACES.items())
)
...
DEFAULT_STIX_SCHEMALOCATIONS = dict(
    itertools.chain(
        iter(list(STIX_NS_TO_SCHEMALOCATION.items())),
        iter(list(EXT_NS_TO_SCHEMALOCATION.items())),
        iter(list(CYBOX_NS_TO_SCHEMALOCATION.items())),
    )
)
...
with ignored(ImportError):
    ...
    prefix_to_ns = dict(
        (prefix, ns) for (ns, prefix) in list(ns_to_prefix.items())
    )
